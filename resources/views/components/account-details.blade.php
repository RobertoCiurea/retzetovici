<div class="flex flex-col lg:flex-row justify-around gap-20 pl-2 lg:mx-10">
        ...
        <div class="flex flex-col items-start gap-20">
            ...
            @enderror
                ...
                <form action="{{ route('user.update', ['userId' => auth()->user()->id]) }}" id="name-form" method="POST" class="hidden">
                    @csrf
                    @method("PUT")
            ...
            <div class="flex flex-col">
                ...
                <form action="{{ route('user.update', ['userId' => auth()->user()->id]) }}" id="email-form" method="POST" class="hidden">
                    @csrf
                    @method("PUT")
            ...
            <form action="{{ route('user.delete', ['userId' => auth()->user()->id]) }}" method="POST">
                @csrf
                @method("DELETE")
